...
- **عبارات عبور** — عبارات عبور به یادماندنی XKCD از لیست 7772 کلمه‌ای EFF بسته‌بندی‌شده
...
محاسبه آنتروپی عبارت عبور بر حسب بیت. اگر `wordlist_size` داده نشود، اندازهٔ واقعی لیست بسته‌بندی‌شده (7772 کلمه، حدود 13 بیت برای هر کلمه) استفاده می‌شود — نه یک مقدار تئوری hardcode‌شده.

```python
from password_generator.passphrase import passphrase_entropy

# 4 کلمه از لیست بسته‌بندی‌شده (اندازه واقعی)
entropy = passphrase_entropy(4)  # ~52 بیت
...
- تحلیلگر قوت یک **heuristic** (الهام‌گرفته از zxcvbn) است؛ با `password-generator[zxcvbn]` می‌توانید مدل واقعی را فعال کنید (`analyze(..., backend="zxcvbn")`)
- لیست کلمهٔ بسته‌بندی‌شده 7772 کلمه دارد؛ entropy بر اساس اندازه واقعی محاسبه می‌شود
...
│   ├── wordlist.txt             # لیست 7772 کلمه‌ای EFF برای عبارات عبور
